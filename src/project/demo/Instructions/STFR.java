 ...
 * @author 一天
 */
public class STFR extends Procedures{
     public boolean EncodeContent(CommonResources insCommonResources,byte[] specificword){
        ...
        System.out.println("Encode SDFR content: " +  res);
    ...
    public boolean GenerateProcedures(CommonResources insCommonResources, byte[] words) {
        ...
        //->FR0,FR1
        if (iFR == 0) {
            operand = insCommonResources.FR0;
            insCommonResources.GUI.AddContectToPane("[CLOCK" + insCommonResources.ClockConter++ + "]FR0:", insCommonResources.setByteToString(insCommonResources.FR0) +"\n");
        }
        else if (iFR == 1) {
            operand = insCommonResources.FR1;
            insCommonResources.GUI.AddContectToPane("[CLOCK" + insCommonResources.ClockConter++ + "]FR1:", insCommonResources.setByteToString(insCommonResources.FR1) +"\n");
        }
        else {
             return false;
        }     
        
        if (false == insCommonResources.setStorge(insCommonResources, operand, EA)) {
            return false;
        }
